...
class SafeSpaceHubArticles extends StatelessWidget {
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      if (mindHubProvider.isRateLimited)
                        const Text(
                            "Cooldown active...", style: TextStyle(color: Colors
                            .red, fontSize: 12)),
                    ],
                  ),
                  ...
                  ListView.builder(
                    ...
                    itemBuilder: (context, index) =>
                        _buildArticleCard(
                            context, mindHubProvider.articles[index]),
                  ),
                ],
              ),
            ),
  ...
  Widget _buildArticleCard(BuildContext context, Article article) {
    return Card(
      ...
      child: InkWell(
        onTap: () {
          Provider
              .of<UserTrackingProvider>(context, listen: false)
              .startTracking('Articles', itemName: article.title);
  ...
  void _showArticleDialog(BuildContext context, Article article) {
    ...
    final width = MediaQuery
        .of(context)
        .size
        .width * (isMobile ? 0.9 : 0.6);
    final height = MediaQuery
        .of(context)
        .size
        .height * (isMobile ? 0.9 : 0.6);
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () {
                          final choice = Provider.of<MindHubProvider>(
                              context, listen: false).getUserChoice(
                              article.id, false);
                          if (choice == null) {
                            _showFeedbackPrompt(context, article.id, false);
                          } else {
                            Provider.of<UserTrackingProvider>(
                                context, listen: false).stopTracking(context);
  ...
  Widget _buildInteractionButtons(BuildContext context, String id,
      bool isVideo) {
    ...
    return Row(
      ...
      children: [
        ...
        _buildChoiceButton(
            ...
            'Cannot Relate',
            itemInteractions['cannot_relate']!,
            choice == 'cannot_relate',
            id,
            isVideo),
      ],
    );
  ...
  Widget _buildChoiceButton(BuildContext context, String key, IconData icon,
      String label, int count, bool isSelected, String id, bool isVideo) {
    final provider = Provider.of<MindHubProvider>(context, listen: false);
    return InkWell(
      onTap: () {
        provider.updateInteraction(id, key, isVideo);
        Provider.of<UserTrackingProvider>(context, listen: false).logEvent(
            context, isVideo ? 'Videos' : 'Articles', id, 'rate_$key');
      },
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(icon, color: isSelected ? MyColors.color2 : Colors.grey,
              size: 20),
          ...
          AlertDialog(
            ...
            actions: [
              TextButton(
                ...
                child: const Text(
                    "Close anyway", style: TextStyle(color: Colors.grey)),
              ),
              ElevatedButton(
                style: ElevatedButton.styleFrom(
                    backgroundColor: MyColors.color2),
                onPressed: () {
                  final provider = Provider.of<MindHubProvider>(
